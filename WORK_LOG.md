...
## 2026-07-19｜Session（續）：全站遷移自訂網域 mentelume.com（ebb4f4d）

### 完成
- **全站網域替換**：31 檔約 200 處 `mori-homepage.pages.dev` → `mentelume.com`（canonical/og:url/JSON-LD/sitemap/robots/頁尾文字/deploy smoke test/quiz template）；sidebiz 的 shared JS 改相對路徑
- **301 轉址**：新增 `functions/_middleware.js`，pages.dev 正式主機名 → mentelume.com（排除 `/api/*` 保護舊快取頁的 POST；preview 部署不受影響）
- **DNS + 綁定**：API 建 apex CNAME（proxied）；Pages 自訂網域用 Playwright 進 dashboard 完成（Mori 首次操作漏按最後的「Activate domain」，這是 522 的根因）
- **線上驗證全過**：mentelume.com 首頁/3 個測驗頁/sitemap/robots 皆 200、`/api/kit/` 405 防護正常、舊網域 301 正確、canonical 正確、瀏覽器載入 0 console error
- 本機 token 權限盤點：可讀寫 zone DNS，**無** Pages:Edit（Pages 網域操作要走 dashboard 或升級 token）

### 完成（續 2）
- **Notion 同步**：測驗漏斗工作室儀表板「現況」區塊改版（新網域/IG 防護/待辦）；測驗總表 27 條「測驗連結」全部換 mentelume.com（驗證 0 殘留）
- **修正 2 條從建立起就錯的 slug**：底層邏輯 thinking-levels→logic-layers、人性18法則 laws-of-human-nature→human-nature（因 SPA fallback 一直偽裝成首頁沒被發現）
- **新增 404.html**：停用 CF Pages SPA fallback，錯誤連結不再回 200 首頁（部署後驗證：未知路徑 404、正常頁 200）

### 完成（續 3）：sidebiz 真正的根因（97b5473）
- Mori 實測回報「起始動畫後只剩粒子」→ 抓到真根因：shared/style.css 全站 `#site{opacity:0}`，每個測驗頁 preloader 收尾要打開它，**sidebiz 在 7/17 改版時弄丟了這行**——內容從頭到尾透明，只剩粒子背景（朋友誤以為是首頁，因為同樣是黑底金粒）
- 修復：preloader fade 時補 `site.style.opacity='1'`；nav 改回設計系統標準結構（`.logo`/`.nav-links`，7/17 改版用了不存在的 class 導致藍色未樣式連結）
- 驗收改用**真截圖**（桌機+390px 手機，本機+線上兩關）：內容完整、Q1 流程正常；教訓寫入 memory/feedback_visual_verification_screenshot.md（DOM 快照看不出透明問題，先前兩輪誤判「已修復」的原因）

### 待辦（Mori 手動）
- **IG bio 連結換成 `https://mentelume.com`**（舊連結會 301，不急但愈早愈好）
- ManyChat / 自動回覆 / Email 序列裡的測驗連結逐步換新網域
- `www.mentelume.com` 未綁定（目前打 www 無回應），要的話再加一條 CNAME + Pages 網域
